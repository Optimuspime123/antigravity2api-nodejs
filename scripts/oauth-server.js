  ...
  if (url.pathname === '/oauth-callback') {
    ...
    if (code) {
      ...
      exchangeCodeForToken(code, port).then(async (tokenData) => {
        ...
        if (config.skipProjectIdFetch) {
          ...
          try {
            ...
            if (projectId === undefined) {
              log.warn('Account is not eligible (projectId unavailable); skipping save');
              res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
              res.end('<h1>Account Not Eligible</h1><p>Unable to obtain projectId, token not saved.</p>');
              setTimeout(() => server.close(), 1000);
              return;
            }
            account.projectId = projectId;
            account.enable = true;
            log.info('Account verification succeeded');
          } catch (err) {
            log.error('Failed to verify account eligibility:', err.message);
            res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
            res.end('<h1>Verification Failed</h1><p>Unable to verify account eligibility. Check console logs.</p>');
            setTimeout(() => server.close(), 1000);
            return;
          }
        }
        ...
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end('<h1>Authorization Successful!</h1><p>The token has been saved. You can close this page.</p>');
        ...
        res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
        res.end('<h1>Failed to obtain token</h1><p>See console for error details.</p>');
...
server.listen(0, () => {
  ...
  log.info('Open the following link in your browser to log in:');
  console.log(`\n${authUrl}\n`);
