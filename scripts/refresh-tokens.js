...
async function refreshAllTokens() {
  ...
  if (!tokens || tokens.length === 0) {
    log.warn('No accounts found for refresh');
    return;
  }
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      ...
      // For 400/403 errors, disable the account to match runtime behavior
      if (statusCode === 400 || statusCode === 403) {
        tokenManager.disableToken(token);
        log.warn(`Account ${i + 1}: token invalid or errored; account disabled`);
      }
    }
  }
