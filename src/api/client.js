...
// Request client: prefer AntigravityRequester, otherwise fall back to axios
let requester = null;
let useAxios = false;
...
async function handleApiError(error, token) {
  ...
  if (status === 403) {
    if (JSON.stringify(errorBody).includes("The caller does not")){
      throw new Error(`Exceeded the model's maximum context. Details: ${errorBody}`);
    }
    tokenManager.disableCurrentToken(token);
    throw new Error(`This account does not have permission and has been disabled automatically. Details: ${errorBody}`);
  }
...
function parseAndEmitStreamChunk(line, state, callback) {
  ...
  try {
    ...
    // When the response ends, send tool calls and usage stats
...
export async function generateAssistantResponse(requestBody, token, callback) {
  ...
  const processChunk = (chunk) => {
    ...
    buffer = lines.pop(); // Keep the final line (it may be incomplete)
...
export async function generateAssistantResponseNoStream(requestBody, token) {
  ...
  // Image generation: convert to markdown format
