...
class TokenManager {
  ...
  async initialize() {
    try {
      ...
      if (this.tokens.length === 0) {
        log.warn('⚠ No available accounts. Add one using the following options:');
        log.warn('  Option 1: run npm run login to sign in');
        log.warn('  Option 2: use the frontend management page to add an account');
      } else {
  ...
  saveToFile(tokenToUpdate = null) {
    try {
      ...
      // If a specific token was provided, update it directly
      if (tokenToUpdate) {
        ...
        // Otherwise update all in-memory tokens
        ...
        if (!token.projectId) {
          if (config.skipProjectIdFetch) {
            ...
            try {
              ...
              log.error(`...${token.access_token.slice(-8)}: Failed to fetch projectId:`, error.message);
