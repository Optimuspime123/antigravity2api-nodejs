...
const DEFAULT_SYSTEM_INSTRUCTION = 'You are a chat robot named Mengmeng, as the name suggests, your personality is soft, glutinous, and cute, specially providing chat and emotional value to users, assisting in novel creation or role-playing';
...
// Restore default proxy system prompt
function restoreDefaultSystemInstruction() {
    ...
    if (textarea) {
        ...
        showToast('Default proxy system prompt restored', 'success');
    }
}
...
// Cache original official system prompt value when loading (to compare if truly modified)
...
// Unlock official system prompt modification
async function unlockOfficialSystemPrompt() {
    const warningMsg = '<span style="color:#ef4444;font-weight:bold;font-size:1rem;">⚠️ Warning! Modifying the official system prompt may cause 429 errors!<br>Confirm changes?</span>';
    ...
    if (password) {
        ...
        showToast('Unlocked, please modify carefully', 'warning');
    }
}

// Handle context System switch changes
function handleContextSystemChange() {
    ...
    if (useContextSystem && mergeSystemPrompt) {
        if (useContextSystem.checked) {
            // When context System is enabled, merge prompts can be freely chosen
            mergeSystemPrompt.disabled = false;
        } else {
            // When context System is disabled, merge prompts automatically turn off and disable
    ...
    try {
        ...
        if (data.success) {
            ...
            const strategyNames = {
                ...
                'quota_exhausted': 'Quota Exhausted Switch',
...
async function loadConfig() {
    try {
        ...
        if (data.success) {
            ...
            // Update merge prompt switch status
            ...
            loadRotationStatus();
            // Default to show only the currently active setting section (for future expansion)
...
function setActiveSettingSection(id, scroll = true) {
    ...
    // Clear search state to avoid interference between "show only one section" and "search filter"
    ...
    if (scroll) {
        ...
        if (el && container) {
            ...
            // Scroll container instead of entire page
...
function lockOfficialSystemPrompt() {
    ...
    if (textarea) {
        ...
        // Clear possible residual inline styles
...
async function saveConfig(e) {
    ...
    // Check if official system prompt has truly been modified
    ...
    // If not modified, remove from envConfig to avoid triggering backend validation
    if (!promptChanged) {
    ...
    // If official system prompt has truly been modified and unlocked with password, include password for backend validation
...
// Page initialization: default to show only one setting section
