...
async function testTokenRotation() {
  ...
  console.log(`Initial state: ${tokenManager.tokens.length} available account(s)\n`);
  ...
  for (let i = 1; i <= totalTests; i++) {
    ...
    console.log(`✓ Retrieved token: ...${tokenId}`);
  ...
  console.log(`Number of unique accounts used: ${usedTokens.size}`);
  console.log('\nUsage per account:');
  usedTokens.forEach((count, tokenId) => {
    console.log(`  ...${tokenId}: ${count} time(s)`);
  });
